...
func CreateVectorClock(nodeId string, timestamp uint64) *pb.VectorClock {
	entry := &pb.VectorClockEntry{
		NodeId:  nodeId,
		Counter: timestamp,
	}

	// Retorna um novo VectorClock contendo apenas essa entrada.
	return &pb.VectorClock{
		Entries: []*pb.VectorClockEntry{entry},
	}
}

// Mesclar todas as versões presentes para uma chave
func MergeStoreEntry(store_entry StoreEntry) *pb.VectorClock {

	mergedClock := MergeVectorClocks(store_entry.Versions[0].VectorClock, nil)

	for i := 1; i < len(store_entry.Versions); i++ {
		mergedClock = MergeVectorClocks(store_entry.Versions[i].VectorClock, mergedClock)
	}
